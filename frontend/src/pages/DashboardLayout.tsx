...
export type Tab = 'dashboard' | 'history' | 'reports' | 'map' | 'assistant';

function DashboardLayout() {
  const [activeTab, setActiveTab] = useState<Tab>(() => {
    const saved = localStorage.getItem('reconsentinel_active_tab');
    const validTabs: Tab[] = ['dashboard', 'history', 'reports', 'map', 'assistant'];
    return (saved && validTabs.includes(saved as Tab)) ? (saved as Tab) : 'dashboard';
  });
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  useEffect(() => {
    localStorage.setItem('reconsentinel_active_tab', activeTab);
  }, [activeTab]);
